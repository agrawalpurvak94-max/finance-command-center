    ...
    >
      <div className="flex items-start justify-between gap-sm">
        <div className="flex min-w-0 flex-1 items-center gap-sm">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-body-sm text-muted-foreground">{card.bankName}</p>
          </div>
        </div>
        <CreditCardHealthBadge health={card.health} />
      </div>

      <div className="flex min-w-0 items-center gap-xs text-body-sm">
        <span
          className={cn(
            'shrink-0 rounded px-1.5 py-0.5 text-label-caps font-semibold uppercase tracking-wide',
      ...
      <div className="min-w-0">
        <div className="flex items-baseline justify-between gap-sm">
          <p className="shrink-0 text-label-caps uppercase text-muted-foreground">Outstanding</p>
        ...
        <p
          ...
          title={formatINR(card.outstanding)}
        >
        ...
        <div className="mt-xs flex items-center justify-between gap-sm text-body-sm text-muted-foreground">
          <span className="min-w-0 truncate">Limit: {formatINR(card.creditLimit)}</span>
          <span className="min-w-0 truncate">Available: {formatINR(card.availableCredit)}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-sm rounded-lg bg-muted/40 p-sm text-table-mono text-xs">
        <div className="min-w-0">
          ...
          <p className="truncate font-semibold text-foreground">{card.statementDate}</p>
        </div>
        <div className="min-w-0">
          <p className="text-muted-foreground">Due Date</p>
          <p className="truncate font-semibold text-foreground">{card.dueDate}</p>
        </div>
        <div className="min-w-0">
          <p className="text-muted-foreground">Minimum Due</p>
          <p className="truncate font-semibold text-foreground">{formatINR(card.minimumDue)}</p>
        </div>
        <div className="min-w-0">
          <p className="text-muted-foreground">Monthly Spend</p>
          <p className="truncate font-semibold text-foreground">{formatINR(card.monthlySpend)}</p>
        </div>
      </div>
      ...
      <div className="mt-auto flex items-center justify-between gap-sm border-t border-border pt-sm text-body-sm text-muted-foreground">
        ...
        <span className="min-w-0 shrink-0 truncate">Last Txn: {card.lastTransactionAt ?? '—'}</span>
      </div>
    </button>
  )
}
